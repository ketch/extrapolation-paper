...
def compare_performance(func, y0, t0, tf, y_ref, problem_name, tol_boundary=(0,6), is_complex=False, nsteps=10e5, solout=(lambda t: t), run_odex_code=False):
    ...
    for i in range(len(tol)):
        ...
        y, infodict = parex.solve(func, [t0, tf], y0, solver='explicit midpoint', atol=tol[i], rtol=tol[i], max_steps=nsteps, diagnostics=True, adaptive=True)
#        y, infodict = parex._explicit_midpoint(func, None, y0, [t0, tf], atol=tol[i], rtol=tol[i], mxstep=nsteps, adaptive="order", full_output=True)
...
def nbod_problem():
    ...
    y_ref = np.loadtxt("reference_nbody.txt")
